...
class VarianceScoreComputer:
    def __init__(self, base_type, debug, entity_db=None, write=True):
        ...
        self.predicate_file_name = settings.DATA_DIRECTORY + f"predicates/predicates.{self.base_type}.tsv"

        # Download the predicate file if it does not exist yet
        if not os.path.isfile(self.predicate_file_name):
            self.download_predicate_mapping()
    ...
    def download_predicate_mapping(self):
        ...
        logger.info(f"Download entities and their predicates for type {self.base_type} to file "
                    f"{self.predicate_file_name} ...")
        tmp_file = settings.DATA_DIRECTORY + "predicates/tmp_file.tsv"
        with open(tmp_file, 'w', encoding="utf8") as out_file:
            subprocess.run(['curl', '-s', 'https://qlever.cs.uni-freiburg.de/api/wikidata',
                            ...
                            '?item ql:has-predicate ?predicate } '],
                           stdout=out_file)
        ...
        with open(tmp_file, "w", encoding="utf8") as out_file:
            sort_process = subprocess.run(['sort', self.predicate_file_name], check=True, capture_output=True)
    ...
    def compute_variances(self):
        ...
        if self.write:
            variance_file_name = settings.DATA_DIRECTORY + f"predicate_variances.{self.base_type}.tsv"
...
def main(args):
    if args.type_qid == "all":
        ...
        for base_type in third_order_types:
            variance_computer = VarianceScoreComputer(base_type, False, entity_db, write=False)
        ...
        variance_computer = VarianceScoreComputer(args.type_qid, args.debug, write=not args.no_write)
        variances = variance_computer.compute_variances()

        if args.generate_plot:
            variance_computer.compute_variance_scores(variances)
            plot_variances(variances, args.type_qid)
        print(variance_computer.timings)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-qid", "--type_qid", type=str, required=True,
    ...
    parser.add_argument("-w", "--no_write", action="store_true",
